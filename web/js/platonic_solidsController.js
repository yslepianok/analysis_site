var myApp = angular.module('platonic_solidsApp', []);

myApp.controller('platonic_solidsController', ['$scope', '$http', function($scope, $http) {
    $http.get("../testing/test?name=platonic_solids").then(function (response) {
  ...
  $scope.url = 'testingresults';
    ...
    console.log("in start platonic_solids testing")
			$scope.flag = 1;
			$scope.flag2 = 1;
			$scope.currentTest = 0;
	}
