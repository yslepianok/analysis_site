var myApp = angular.module('activity_levelsApp', []);

myApp.controller('activity_levelsController', ['$scope', '$http', function($scope, $http) {
    $http.get("../testing/test?name=activity_levels").then(function (response) {
  ...
  $scope.url = 'testingresults';
    ...
    console.log("in start activity_levels testing")
			$scope.flag = 1;
			$scope.flag2 = 1;
			$scope.currentTest = 0;
	}
